module.exports = async function handler(req,res){
  ...
  try{
    ...
    const r=await fetch('https://zzjcimwqttlqrcjiuffm.supabase.co/functions/v1/job-ops',{method:'POST',headers:{apikey:'sb_publishable_kexH74ejNOb7IM-Lzikouw_22xRKSR7',authorization:auth,'content-type':'application/json'},body:JSON.stringify(payload)});
  ...
  }catch(e){res.statusCode=400;res.setHeader('content-type','application/json');res.end(JSON.stringify({error:e?.message||'Job Operations proxy failed'}))}
}
